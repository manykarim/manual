...
The default titles for [logs](results.md#splitting-logs) and [reports](post-processing.md#creating-reports-logs-and-output-files) are generated by prefixing
...
Messages in [log files](results.md#log) can have different log levels. Some of the
...
`FOR`
: Flatten [FOR loops](../syntax/control.md#for-loops) fully.

`WHILE`
: Flatten [WHILE loops](../syntax/control.md#while-loops) fully.
...
When [combining outputs](#combining-outputs) using Rebot, it is possible to set the start
...
Full error messages are always visible in [log files](results.md#log) as messages of
the failed keywords.
