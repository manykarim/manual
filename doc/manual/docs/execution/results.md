...
Messages in [log files](#log-file) can have different log levels. Some of the
messages are written by Robot Framework itself, but also executed
keywords can [log information](https://wiki.jenkins-ci.org/display/JENKINS/Robot+Framework+Plugin) using different levels. The available
log levels are:
...
`ERROR`
   ...
   the Test Execution Errors section in log files](../creating-test-data/creating-test-suites.md#free-suite-metadata), but they
   do not affect test case statuses. If the [--exitonerror option is enabled](../extending/creating-test-libraries.md#logging-information),
...
teardown](test-execution.md#test-teardown).
...
cases](test-execution.md#stopping-on-parsing-or-execution-error)::
...
tags. The combined tags are specified using [tag patterns](basic-usage.md#tag-patterns) where
...
`tag` may be a single tag, but more commonly a [simple pattern](basic-usage.md#simple-pattern)
...
documentation to, and it can also be a [simple pattern](basic-usage.md#simple-pattern) matching
...
they contain [FOR loops](../creating-test-data/control-structures.md#for-loops) or other constructs that repeat certain keywords
multiple times.
...
They can be used both when [executing test cases](basic-usage.md#executing-test-cases) and when [post-processing
...
or warnings](basic-usage.md#errors-and-warnings-during-execution) are not removed except when using the `ALL` mode.
...
`TAG:<pattern>`
   ...
   Can be used both with [library keyword tags](configuring-execution.md#by-tag-names) and [user keyword tags](../creating-test-data/creating-user-keywords.md#user-keyword-tags).
...
Removing keywords is done after parsing the [output file](#output-file) and generating
an internal model based on it. Thus it does not reduce memory usage as much
as [flattening keywords](#flattening-keywords).
...
`FOR`
   Flatten [FOR loops](../creating-test-data/control-structures.md#for-loops) fully.

`WHILE`
   Flatten [WHILE loops](../creating-test-data/control-structures.md#while-loops) fully.
...
Flattening keywords is done already when the [output file](#output-file) is parsed
...
the execution time. This can be done only on an user keyword level by defining the [reserved tag](../extending/creating-test-libraries.md#errors-and-warnings)
`robot:flatten` as a [keyword tag](../extending/creating-test-libraries.md#keyword-tags). Using this tag will work similarly as the command line
...
When [combining outputs](post-processing.md#combining-outputs) using Rebot, it is possible to set the start
...
Full error messages are always visible in [log files](#log-file) as messages of
the failed keywords.
...
test data](configuring-execution.md#programmatic-modification-of-test-data) that can be enabled with the `--prerunmodifier` option.
...
class ExecutionTimeChecker(SuiteVisitor):
    ...
    def [__init](#init)(self, max_seconds: float):
...
A system log has the same [log levels](#log-levels) as a normal log file, with the
...
example below.  Possible [unexpected errors and warnings](../creating-test-data/creating-test-cases.md#reserved-tags) are
