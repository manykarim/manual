...
User keywords and variables in [suite files](creating-test-suites.md#suite-files) and [suite
initialization files](creating-test-suites.md#suite-initialization-files) can only be used in files where they are
...
as when creating suite files and [supported file formats](test-data-syntax.md#supported-file-formats) are the same
...
also makes them somewhat more complicated than [Variable sections](variables.md#variable-sections).
...
file name](advanced-features.md#handling-keywords-with-same-names) without the extension (for example, *myresources.Some
...
Keywords created in a resource file can be [documented](#documented) using
`[Documentation]` setting. The resource file itself can have
`Documentation` in the Setting section similarly as [suites](creating-test-suites.md#creating-test-suites).
...
the same way as when parsing [reStructuredText suite files](creating-user-keywords.md#user-keyword-name-and-documentation).
...
Resource files can be created using [JSON](test-data-syntax.md#json-format) the [same way as suite files](creating-test-suites.md#suite-name).
...
```
Resource files can be converted to JSON using [ResourceFile.to_json](test-data-syntax.md#restructuredtext-format) and
recreated using [ResourceFile.from_json](test-data-syntax.md#json-format):
...
the command line](https://robot-framework.readthedocs.io/en/master/autodoc/robot.running.html#robot.running.model.ResourceFile.to_json), but variable files allow creating them dynamically
...
[Getting variables from a special function](variables.md#variable)
...
Alternatively variable files can be implemented as [classes](https://robot-framework.readthedocs.io/en/master/autodoc/robot.running.html#robot.running.model.ResourceFile.from_json)
...
method. Variable files can also be created as [YAML](#yaml) and [JSON](test-data-syntax.md#json-format).
...
a path to the file same way as [resource files are imported](variables.md#command-line-variables) using
...
can also be imported using the module name [similarly as libraries](#implementing-variable-file-as-a-class).
...
```
If a [variable file accepts arguments](#variable-file-as-yaml), they are specified after the path
...
available in all test data files, similarly as [individual variables](#variable-file-as-json)
...
](#if-there-is-a-large-number-of-other-attributes-instead-of-prefixing-them-all-it-is-often-easier-to-use-a-special-attribute)all__` and give it a list of attribute names to be processed
as variables.

```python
import math

[__all](#all) = ['AREA1', 'AREA2']
...
``[
!!! note
    The ](#note-the)all__` attribute is also, and originally, used
